...
func watchAndExec(config *WatchConfig) (int, error) {
	...
	defer close(quitCh)
	go watch(
		client, config.Prefix+"/env", pairCh, errCh, quitCh,
	...
	for {
		...
		for _, pair := range pairs {
			k := strings.TrimPrefix(pair.Key, config.Prefix+"/env")
		...
		monitor, _ := NewMonitor(config.Prefix, client)
		monitor.storeMetadata(config.Cmd)
		...
		// NOTE Use errCh as well ?
		exitCh = make(chan int, 1)

		// Acquire process metrics and store it in influx db
		go monitor.watchMetrics(cmd.Process.Pid, quitCh, 5000)
